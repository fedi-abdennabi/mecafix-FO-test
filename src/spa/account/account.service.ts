...
export const useAccount = (
  ...
  const isSuperAdmin = account?.role.roleName==="superAdmin" ; 
  const isAdmin = account?.role.roleName==="admin";
  const isClient =account?.role.roleName==="client";
  const isManager = false;
  return { account, isAdmin, isClient, isManager, isSuperAdmin, ...rest };
};

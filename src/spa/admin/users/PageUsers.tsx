...
const UserActions = ({ user, ...rest }: UserActionProps) => {
  ...
  const { mutate: userDeActivate, ...userDeActivateData } = useDeActivateUser({
    onSuccess: () => {
      toastSuccess({
  ...
  const deActivateUser = () => userDeActivate(user);
  const isActionsLoading = userActivateData.isLoading;
  const isDeActivateActionsLoading = userDeActivateData.isLoading;

  return (
    <Menu isLazy placement="left-start" {...rest}>
      <MenuButton
        as={ActionsButton}
        isLoading={isActionsLoading || isRemovalLoading || isDeActivateActionsLoading}
      />
      <Portal>
        <MenuList>
          {!user.activated && (

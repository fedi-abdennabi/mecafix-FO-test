...
export const useDefaultSubCategoriesList = (
    ...
    const fetchSubCategories = (): Promise<DefaultSubCategoriesListResult> => {
        return Axios.get(`/admin/defaultSubcategory/${id}`, { params: { page } });
    };
...
export const useDefaultSubCategoriesCreate = (
    ...
    return useMutation((payload) => Axios.post(`/admin/defaultSubcategory/${id}/create`, payload), {
        ...config,
        onSuccess: (...args) => {
...
export const useDefaultSubCategoriesDelete = (
    ...
    const queryClient = useQueryClient();
    return useMutation(({ id }) => Axios.delete(`/admin/defaultSubcategory/${id}`), {
        ...config,
        onSuccess: (...args) => {
            queryClient.invalidateQueries(DefaultSubCategoriesKeys.all._def);
            config?.onSuccess?.(...args);
...
export const useDefaultSubCategoriesUpdate = (
    ...
    const queryClient = useQueryClient();
    return useMutation((payload) => Axios.put(`/admin/defaultSubcategory/update/${id}`, payload), {
        ...config,
        onSuccess: (...args) => {
            queryClient.invalidateQueries(DefaultSubCategoriesKeys.all._def);
            config?.onSuccess?.(...args);
...
export const useDefaultSubCategoryDetails = (
    ...
    const result = useQuery(
        DefaultSubCategoriesKeys.DefaultSubCategoriesDetails().queryKey,
        (): Promise<DefaultSubCategories> => Axios.get(`/admin/defaultSubcategory/details/${id}`),
        config
    );
    return {

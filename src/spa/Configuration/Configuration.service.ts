...
export const useConfiguration = (
  ...
  const result = useQuery(
    ConfigurationKeys.all().queryKey,
    (): Promise<Configuration> => Axios.get(`/superAdmin/configuration`),
    {
      ...config,
    }
  );
...
export const useConfigurationUpdate = (
  ...
  return useMutation((payload) => Axios.put(`/superAdmin/configuration`, payload), {
    ...config,
    onSuccess: (...args) => {
...
export const useConfigurationCreate = (
  ...
  const queryClient = useQueryClient();
  return useMutation((payload) => Axios.post('/superAdmin/configuration', payload), {
    ...config,
    onSuccess: (...args) => {
      queryClient.invalidateQueries(ConfigurationKeys.all._def);
      config?.onSuccess?.(...args);

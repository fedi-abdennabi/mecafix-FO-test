...
export const usePackList = (
  ...
  const result = useQuery(
    ...
    (): Promise<PackListResult> =>
      Axios.get(`superAdmin/pack`, { params: { page } }),
...
export const usePack = (
  ...
  const result = useQuery(
    ...
    (): Promise<Pack> => Axios.get(`/superAdmin/pack/${id}`),
    {
      ...config,
    }
  );
...
export const usePackUpdate = (
  ...
  return useMutation((payload) => Axios.put(`/superAdmin/pack/${id}`, payload), {
    ...config,
    onSuccess: (...args) => {
...
export const usePackCreate = (
  ...
  const queryClient = useQueryClient();
  return useMutation((payload) => Axios.post('/superAdmin/pack', payload), {
    ...config,
    onSuccess: (...args) => { 
...
export const usePackRemove = (
  ...
  const queryClient = useQueryClient();
  return useMutation((): Promise<void> => Axios.delete(`/superAdmin/pack/${id}`), {
    ...config,
    onSuccess: (...args) => {
      queryClient.invalidateQueries(PackKeys.all._def);
      config?.onSuccess?.(...args);
...
export const usePackDetails = (
  ...
  const result = useQuery(
    PackKeys.PackUserDetails().queryKey,
    (): Promise<PackDetails> => Axios.get(`/superAdmin/pack/${userId}-details`),
    {
